angular.module('WebApiApp').controller("ModelGioiTinhHandlerController", function ($rootScope, $scope, $http, $uibModalInstance) {
    ...
    $scope.SaveModal = function () {
        ...
        if (typeof $scope.item.Id == 'undefined' || $scope.item.Id == 0) {
            ...
            $http({
                method: 'PUT',
                url: '/api/GioiTinhs/' + $scope.item.Id,
                data: $scope.item
            }).then(function successCallback(response) {
    ...
    $scope.ValidOnlyCode = function (FCode) {
        ...
        $http({
            method: 'GET',
            url: '/api/CheckValidGioiTinh/' + FCode,
        }).then(function successCallback(response) {
...
angular.module('WebApiApp').controller('GioiTinhController', ['$rootScope', '$scope', '$http', '$cookies', '$uibModal', '$settings', function ($rootScope, $scope, $http, $cookies, $uibModal, $settings) {
    ...
    $rootScope.Load = function () {
        ...
        $http({
            method: 'GET',
            url: '/api/GioiTinhs?pageNumber=' + $scope.Paging.currentPage + '&pageSize=' + $scope.Paging.pageSize + '&searchKey=' + $scope.Paging.searchKey
        }).then(function successCallback(response) {

            $scope.Areas = response.data.gioiTinhs;
