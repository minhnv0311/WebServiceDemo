angular.module('WebApiApp').controller("ModelChucDanhHandlerController", function ($rootScope, $scope, $http, $uibModalInstance) {
    ...
    $scope.SaveModal = function () {
        ...
        if (typeof $scope.item.Id == 'undefined' || $scope.item.Id == 0) {
            ...
            $http({
                method: 'PUT',
                url: '/api/ChucDanhs/' + $scope.item.Id,
                data: $scope.item
            }).then(function successCallback(response) {
    ...
    $scope.ValidOnlyCode = function (FCode) {
        ...
        $http({
            method: 'GET',
            url: '/api/CheckValidActivityInspect/' + FCode,
        }).then(function successCallback(response) {
...
angular.module('WebApiApp').controller('ChucDanhController', ['$rootScope', '$scope', '$http', '$cookies', '$uibModal', '$settings', function ($rootScope, $scope, $http, $cookies, $uibModal, $settings) {
    ...
    $rootScope.Load = function () {
        ...
        $http({
            method: 'GET',
            url: '/api/ChucDanhs?pageNumber=' + $scope.Paging.currentPage + '&pageSize=' + $scope.Paging.pageSize + '&searchKey=' + $scope.Paging.searchKey
        }).then(function successCallback(response) {

            $scope.Areas = response.data.dt;
