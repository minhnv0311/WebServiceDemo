angular.module('WebApiApp').controller("ModelTrangThaiHandlerController", function ($rootScope, $scope, $http, $uibModalInstance) {
    ...
    $scope.SaveModal = function () {
        ...
        if (typeof $scope.item.Id == 'undefined' || $scope.item.Id == 0) {
            ...
            $http({
                method: 'PUT',
                url: '/api/TrangThais/' + $scope.item.Id,
                data: $scope.item
            }).then(function successCallback(response) {
    ...
    $scope.ValidOnlyCode = function (FCode) {
        ...
        $http({
            method: 'GET',
            url: '/api/CheckValidTrangThai/' + FCode,
        }).then(function successCallback(response) {
...
angular.module('WebApiApp').controller('TrangThaiController', ['$rootScope', '$scope', '$http', '$cookies', '$uibModal', '$settings', function ($rootScope, $scope, $http, $cookies, $uibModal, $settings) {
    ...
    $rootScope.Load = function () {
        ...
        $http({
            method: 'GET',
            url: '/api/TrangThais?pageNumber=' + $scope.Paging.currentPage + '&pageSize=' + $scope.Paging.pageSize + '&searchKey=' + $scope.Paging.searchKey
        }).then(function successCallback(response) {

            $scope.Areas = response.data.TrangThai;
