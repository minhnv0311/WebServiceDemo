angular.module('WebApiApp').controller('NewsController', ['$q', '$rootScope', '$scope', '$http', '$cookies', '$uibModal', '$settings', '$timeout', function ($q, $rootScope, $scope, $http, $cookies, $uibModal, $settings, $timeout) {
    ...
    $scope.SaveNews = function () {
        ...
        if ($scope.item.FCreateTime == "") $scope.item.FCreateTime = null;
        ...
        $http({
            ...
            $http({
                ...
                params: { news: response.data, CodeTags: JSON.stringify($scope.Tags) },
            }).then(function successCallback(response) {
            ...
            if (!angular.isUndefined(response.data.ModelState.Tags)) {
                var text = '';
                text += response.data.ModelState.Tags[0] + "<br>";
                toastr.error(text, "Lỗi cập nhật dữ liệu");
            }
