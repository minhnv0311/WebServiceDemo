...
namespace WebApiCore.Controllers.News
{
    ...
    public class LinksController : ApiController
    {
        ...
        [HttpPost]
        [Route("api/CMS_Links")]
        public IHttpActionResult CMS_Links(CMS_Links oNews)
        {
            try
            {
                Validate(oNews);
                ...
                if (oNews.Id == 0)
                {
                    ...
                    if (string.IsNullOrEmpty(oNews.FCode))
                        oNews.FCode = "LINK_" + DateTime.Now.Ticks;
        ...
        [HttpDelete]
        [Route("api/CMS_Links/Delete")]
        public IHttpActionResult CMS_LinksDelete(long id)
        {
            try
            {
                string branchCode = HttpContext.Current.Request.Headers["x-company"];
                ...
                var obj = db.CMS_Links.Where(x => x.FInUse == true && x.FBranchCode == branchCode && x.FLanguage == language && x.Id == id).FirstOrDefault();
                if (obj != null)
                {
                    obj.FInUse = false;
        ...
        public IHttpActionResult CMS_ListLinks(int pageNumber, int pageSize, string searchKey)
        {
            try
            {
                ...
                var data = db.CMS_Links.Where(x => x.FInUse == true && x.FBranchCode == branchCode && x.FLanguage == language
                ...
                var total = db.CMS_Links.Where(x => x.FInUse == true && x.FBranchCode == branchCode && x.FLanguage == language
        ...
        public IHttpActionResult GetById(long id)
        {
            try
            {
                ...
                var data = db.CMS_Links.Where(x => x.FInUse == true && x.FBranchCode == branchCode && x.FLanguage == language && x.Id == id).FirstOrDefault();
                return Ok(data);
        ...
        private void Validate(CMS_Links item)
        {
