//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace WebApiCore.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class CMS_Race
    {
        public long Id { get; set; }
        public string FCode { get; set; }
        public string FName { get; set; }
        public string FDescription { get; set; }
        public Nullable<System.DateTime> FCreateTime { get; set; }
        public Nullable<System.DateTime> FUpdateTime { get; set; }
        public string FUserCreate { get; set; }
        public string FUserApprove { get; set; }
        public Nullable<System.DateTime> FApproveTime { get; set; }
        public string FStatus { get; set; }
        public Nullable<int> FIndex { get; set; }
        public Nullable<int> FLevel { get; set; }
        public Nullable<bool> FInUse { get; set; }
        public string FBranchCode { get; set; }
        public string FUserUpdate { get; set; }
        public string FLanguage { get; set; }
        public string Menu { get; set; }
        public string Image { get; set; }
        public Nullable<bool> IsShowSortContent { get; set; }
        public Nullable<System.DateTime> StartDate { get; set; }
        public Nullable<System.DateTime> EndDate { get; set; }
        public string SortContent { get; set; }
        public string Content { get; set; }
        public Nullable<bool> IsHotNews { get; set; }
        public Nullable<bool> IsTopNews { get; set; }
        public Nullable<bool> IsImportantNews { get; set; }
        public Nullable<bool> IsLiveNews { get; set; }
        public string RouterProfile { get; set; }
        public string RaceMap { get; set; }
        public string Distance { get; set; }
        public string LowestPoint { get; set; }
        public string Uphill { get; set; }
        public string Flat { get; set; }
        public string SteepestUphill { get; set; }
        public string LongestUphill { get; set; }
        public string TotalAscentDescent { get; set; }
        public string HighgestPoint { get; set; }
        public string Downhill { get; set; }
        public string MaxHeightGain { get; set; }
        public string SteepestDownhill { get; set; }
        public string LongestDownhill { get; set; }
        public string UrlRegister { get; set; }
    }
}
